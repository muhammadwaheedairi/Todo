...
import { Search, Calendar as CalendarIcon, ChevronLeft, ChevronRight, Menu } from 'lucide-react';
  ...
  return (
    <header className="h-20 md:h-24 bg-white border-b border-gray-100 flex items-center justify-between px-4 md:px-8 sticky top-0 z-20 shrink-0">
      <div className="flex items-center gap-4 md:gap-8 flex-1 min-w-0">
        ...
        <h1 className="text-xl md:text-3xl font-bold text-[#F45B5B] whitespace-nowrap hidden xs:block">{activeTab}</h1>
        ...
        <div className="relative max-w-xl w-full">
          <input 
            type="text" 
            placeholder="Search..." 
            className="w-full bg-[#F8FAFC] border-none rounded-full py-2.5 md:py-3.5 px-4 md:px-6 pr-12 md:pr-14 focus:ring-2 focus:ring-[#F45B5B]/20 transition-all text-xs md:text-sm outline-none"
          />
          <button className="absolute right-1 top-1 bottom-1 bg-[#F45B5B] text-white p-2 md:p-2.5 rounded-full hover:bg-[#E04A4A] transition-colors">
            <Search size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-4 relative shrink-0">
        ...
        <div className="ml-2 text-right leading-none hidden md:block">
          <div className="text-gray-500 text-xs font-medium">
            {new Intl.DateTimeFormat('en-US', { weekday: 'long' }).format(new Date())}
          </div>
          <div className="text-[#3BAFDA] text-sm font-bold mt-0.5">
